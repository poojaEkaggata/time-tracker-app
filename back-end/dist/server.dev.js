...
}); //const db = mysql.createConnection({host:"172.17.0.1",user:"root",password:"",database:"ekatimesheet"});
//console.log(db);
...
app.get('/login/checkLoggedInStatus', (req, res) => 
{
  console.log(loggedInStatus);
  res.json({ status: loggedInStatus });
}); */
...
app.post('/login', function (req, res) {
  ...
  db.query(getUserSql, [req.body.email], function (err, results) {
    ...
    if (results.length > 0) {
      ...
      _bcryptjs["default"].compare(inputedPassword, storedHashedPassword, function (error, response) {
        if (response) {
...
app.post('/home', function (req, res) {
  ...
  _bcryptjs["default"].hash(password.toString(), salt, function (err, hashedPassword) {
    if (err) {
      return res.json({
...
/* app.get('/user_home/timesheets/date_events', async (req, res) => 
{
  const { month, year } = req.query;
  try 
  {
    //const query = 'SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = $1 AND EXTRACT(YEAR FROM fromdate) = $2';
    const query = 'SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = ? AND EXTRACT(YEAR FROM fromdate) = ?';
    console.log(query);
...
app.get('/user_home/timesheets/date_events', function _callee12(req, res) {
  var userId, _req$query, month, year, parsedMonth, parsedYear, query, _ref, rows;
    ...
    while (1) {
      switch (_context13.prev = _context13.next) {
        ...
        case 11:
          ...
          // const query = 'SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = $1 AND EXTRACT(YEAR FROM fromdate) = $2';
          // const { rows } = await db.query(query, [month, year]);
          query = "SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = ".concat(parsedMonth, " AND EXTRACT(YEAR FROM fromdate) = ").concat(parsedYear, " AND user_id=userId");
          console.log(query);
          _context13.next = 16;
        ...
        case 22:
          ...
          console.error('Error fetching timesheets:', _context13.t0);
          res.status(500).json({
...
/* const PORT = process.env.NODE_LOCAL_PORT || process.env.NODE_DOCKER_PORT;
app.listen(PORT, () => {
