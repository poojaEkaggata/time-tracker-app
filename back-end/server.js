    ...
    if(results.length > 0) 
    {
      ...
      bcrypt.compare(inputedPassword,storedHashedPassword,(error,response)=>
      {
        if(response)
        {
          ...
          return res.status(200).json({ loggedIn: true, message: 'Login successful', user: { email: userEmail, user_id: userId }, userEmail, userId });
        }
        else
        {
          return res.status(401).json({ loggedIn: false, message: 'Incorrect password' });
        }
      });
    }
    else
    {
      return res.status(401).json({ loggedIn: false, message: 'User does not exist!' });
    }
  });
});
  ...
  if(userId)
  {
    ...
    else
    {
      try 
      {
        ...
        // const query = 'SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = $1 AND EXTRACT(YEAR FROM fromdate) = $2';
        // const { rows } = await db.query(query, [month, year]);
        const query = `SELECT * FROM timesheet WHERE EXTRACT(MONTH FROM fromdate) = ${parsedMonth} AND EXTRACT(YEAR FROM fromdate) = ${parsedYear} AND user_id=userId`;
        console.log(query);
