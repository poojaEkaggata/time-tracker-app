...
function MonthView({ date }) 
{
  ...
  useEffect(() => 
  {
    const fetchData = async () => 
    {
      try 
      {
        const response = await fetch(`/user_home/timesheets/date_events?month=${date.getMonth()+1}&year=${date.getFullYear()}`);
        if(!response.ok) 
        {
  ...
  const renderDayTimesheets = (day) => 
  {
    const dayTimesheets = timesheets.filter(timesheet => timesheet.day === day);
    return (
      <>
        <ul>
          ...
          (
            <div key={index}>
              <li>{timesheet.fromtime}</li>
              <li>{timesheet.activity}</li>
            </div>
          ))}
        </ul>
      </>
